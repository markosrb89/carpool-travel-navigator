...
export const rideFormSchema = z
  .object({
    ...
    pickupWindow: z
      .number()
      .min(5, "Minimum pickup window is 5 minutes")
      .max(60, "Maximum pickup window is 60 minutes"),
  ...
  .refine(
    ...
    {
      message:
        "Please select at least one recurring day and ensure end date is after start date",
      path: ["recurringDays"],
    }
  );
